...
export class AiRideService {
   ...
   *
   * @param {string} userText - Free-form input from the user
  ...
  _buildSystemPrompt() {
    ...
    return `You are a structured data extractor for a cycling ride scheduling bot.
...
Return ONLY valid JSON, no markdown, no explanation. Omit fields that are not mentioned.`;
  ...
  _buildUserMessage(userText, options) {
    const { isUpdate, originalText, followUpText } = options;
