...
• Управление дефолтами и настройками отдельных поездок
• Привязка закрытых групп к поездкам
...
5. Настройте дефолты через /settings, если хотите другое поведение для будущих поездок
6. Поделитесь поездкой в другие чаты через /shareride (бот должен быть добавлен в другой чат до публикации; /shareride@botname работает, когда короткая команда недоступна)
7. Все участники и изменения синхронизируются автоматически!
...
Поля заполняются автоматически: название, дата, место встречи, категория, ссылки на маршрут, расстояние, длительность, крейсерская скорость (из скоростных темповых групп), организатор (название клуба) и дополнительная информация (ссылка на событие + описание + темповые группы). Средняя скорость движения из темповых групп не выводится.
...
Переданные настройки поездки будут объединены с текущими настройками этой поездки.
...
Существующую поездку можно продублировать 4 способами:
...
При дублировании своей поездки копируются ее настройки. При дублировании чужой поездки используются ваши текущие дефолты.
...
Используйте /settings в личном чате, чтобы управлять дефолтами для будущих поездок.
Используйте /settings #rideId, ответьте на сообщение поездки командой /settings или нажмите кнопку «Настройки» в личной копии поездки создателя, чтобы управлять конкретной поездкой.
Доступные настройки:
• Уведомления об участии — будет ли создатель получать личные уведомления, когда участники записываются, думают или отказываются.
• Разрешение репостов — могут ли пользователи кроме создателя публиковать поездку через /shareride.
Дефолты применяются только к новым поездкам. Настройки поездки влияют только на эту поездку.

<b>🧭 Кнопки управления в личном чате</b>
В личном сообщении своей поездки создатель видит кнопки: редактировать, дублировать, удалить, отменить/возобновить, посмотреть участников и открыть настройки.

<b>📢 Публикация поездки</b>
По умолчанию публиковать поездку в другой чат может только создатель. Создатель может разрешить репосты другим пользователям в /settings.
...
Бот переименует группу по названию и дате поездки, опубликует и закрепит информацию о поездке и будет автоматически добавлять/удалять участников по мере изменения их статуса.
Одна группа может быть привязана только к одной поездке одновременно.
